...
就是一个滑动窗口，思路是：向右移动右指针，直到覆盖 t 中的所有字符；一旦所有字符都被覆盖，移动左指针并确保所有字符仍然被覆盖，以最小化子数组的大小。

但是要判断所有字符都被覆盖，如果每次都把所有的字符的记录都对比一遍，时间复杂度就变成了$O(m*k)$，m是滑动窗口改变的次数，k为字符集的大小。

解决办法是：使用`less` 表示：**当前滑动窗口中，出现次数仍小于 t 中对应字符出现次数的「不同字符的种类数」**。

比如 t="ABC"，初始时窗口为空，3 种字符都不满足要求，所以`less=3`；当窗口内 "A" 的次数达到 1（等于 t 中次数），`less`就减 1（变成 2）；当 "A"、"B"、"C" 的次数都≥t 中次数时，`less=0`，说明窗口有效。
